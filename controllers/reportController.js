...
//     const [totals] = await Sale.aggregate([{ $match: match }, { $group: { _id: null, revenue: { $sum: '$totalProfit' }, count: { $sum: 1 }, discount: { $sum: '$discount' }, tax: { $sum: '$tax' } } }]);
...
const getProfitReport = async (req, res, next) => {
  try {
    ...
    const grossProfit = profit - returns;
...
//       {
//         $lookup: {
//           from: 'productcategories', // collection name in MongoDB
//           localField: '_id',
//           foreignField: '_id',
//           as: 'category',
//         },
